				...
				class RBTree
				{
					public:
						...
						RBTree(const KeyTypeRB& k, const ValueTypeRB& v) :
								LeftChild(0), RightChild(0), Parent(0), Key(k), Value(
										v), IsRed(true)
						{
						}
			...
			public:
				...
				class ParentFirstIterator
				{
					public:
						...
						ParentFirstIterator& operator=(
								const ParentFirstIterator& src)
						{
							Root = src.Root;
							Cur = src.Cur;
							return (*this);
				...
				class AccessClass
				{
					...
					private:
						...
						AccessClass(map& tree, const KeyType& key) :
								Tree(tree), Key(key)
						{
						}
