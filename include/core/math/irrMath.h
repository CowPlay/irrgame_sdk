...
#undef PI
		...
		inline bool equals(const f32 a, const f32 b, const f32 tolerance =
				ROUNDING_ERROR_f32)
		{
		...
		//! returns if a equals b, taking an explicit rounding tolerance into account
		inline bool equals(const u32 a, const u32 b, const s32 tolerance =
				ROUNDING_ERROR_S32)
		{
			return (a + tolerance >= b) && (a - tolerance <= b);
		}
...
#define REALINLINE inline
		...
		REALINLINE u32 if_c_a_else_b(const s32 condition, const u32 a,
				const u32 b)
		{
		...
		//! conditional set based on mask and arithmetic shift
		REALINLINE u16 if_c_a_else_b(const s16 condition, const u16 a,
				const u16 b)
		{
