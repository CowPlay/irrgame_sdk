		...
		class vector3d
		{
			...
			public:
				...
				bool operator<=(const vector3d<T>&other) const
				{
					return (X < other.X || core::equals(X, other.X))
							...
							|| (core::equals(X, other.X)
									...
									&& (Z < other.Z || core::equals(Z, other.Z)));
				}
				...
				bool operator<(const vector3d<T>&other) const
				{
					return (X < other.X && !core::equals(X, other.X))
							|| (core::equals(X, other.X) && Y < other.Y
									&& !core::equals(Y, other.Y))
							|| (core::equals(X, other.X)
									&& core::equals(Y, other.Y) && Z < other.Z
				...
				vector3d<T> getHorizontalAngle() const
				{
					...
					const f32 tmp = (atan2((f32) X, (f32) Z) * RADTODEG);
					angle.Y = (T) tmp;
					...
					const f32 z1 = core::squareroot(X * X + Z * Z);

					angle.X = (T) (atan2((f32) z1, (f32) Y) * RADTODEG - 90.0);
				...
				vector3d<T> getSphericalCoordinateAngles()
				{
					...
					if (length)
					{
						if (X != 0)
						{
							angle.Y = (T) (atan2((f32) Z, (f32) X) * RADTODEG);
						}
						else if (Z < 0)
							angle.Y = 180;
				...
				{
					const f32 cr = cos(core::DEGTORAD * X);
					const f32 sr = sin(core::DEGTORAD * X);
					const f32 cp = cos(core::DEGTORAD * Y);
					const f32 sp = sin(core::DEGTORAD * Y);
					const f32 cy = cos(core::DEGTORAD * Z);
					const f32 sy = sin(core::DEGTORAD * Z);

					const f32 srsp = sr * sp;
					const f32 crsp = cr * sp;

					const f32 pseudoMatrix[] =
					{ (cp * cy), (cp * sy), (-sp), (srsp * cy - cr * sy), (srsp
							* sy + cr * cy), (sr * cp), (crsp * cy + sr * sy),
							(crsp * sy - sr * cy), (cr * cp) };
