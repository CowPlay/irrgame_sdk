	...
	namespace video
	{
		...
		class SColor
		{
			public:
				...
				{
					return 0.5f
							* (core::max_(core::max_(getRed(), getGreen()),
									getBlue())
									+ core::min_(
											core::min_(getRed(), getGreen()),
											getBlue()));
				}
				...
				{
					return 0.3f * getRed() + 0.59f * getGreen()
							+ 0.11f * getBlue();
				}
				...
				{
					return SColor(
							core::min_(getAlpha() + other.getAlpha(), 255u),
		...
		{
			const f32 maxVal = (f32) core::max_(color.getRed(),
					color.getGreen(), color.getBlue());
			const f32 minVal = (f32) core::min_(color.getRed(),
					color.getGreen(), color.getBlue());
		...
		inline u32 SColorHSL::toRGB1(f32 rm1, f32 rm2, f32 rh) const
		{
			...
			if (rh < 60.0f * core::DEGTORAD)
				...
				rm1 = rm1
						+ (rm2 - rm1) * ((240.0f * core::DEGTORAD) - rh)
								/ (60.0f * core::DEGTORAD);
