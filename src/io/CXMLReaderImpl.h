		...
		class CXMLReaderImpl: public IIrrXMLReader<char_type, superclass>
		{
			public:
				...
				CXMLReaderImpl(IFileReadCallBack* callback,
						...
						TextData(0), P(0), TextBegin(0), TextSize(0), CurrentNodeType(
								EXN_NONE), SourceFormat(ETF_ASCII), TargetFormat(
								ETF_ASCII)
				{
					if (!callback)
						return;
				...
				virtual bool read()
				{
					...
					if (P && (unsigned int) (P - TextBegin) < TextSize - 1
							&& *P != 0)
					{
			...
			private:
				...
				void parseComment()
				{
					...
					P -= 3;
					NodeName = core::string<char_type>(pCommentBegin + 2,
				...
				void parseOpeningXMLElement()
				{
					...
					while (*P != L'>')
					{
						...
						else
						{
							if (*P != L'/')
							{
								...
								SAttribute attr;
								attr.Name = core::string<char_type>(
										attributeNameBegin,
					...
					NodeName = core::string<char_type>(startName,
				...
				void parseClosingXMLElement()
				{
					...
					NodeName = core::string<char_type>(pBeginClose,
							(int) (P - pBeginClose));
					++P;
				}
				...
				bool parseCDATA()
				{
					...
					if (cDataEnd)
						NodeName = core::string<char_type>(cDataBegin,
				...
				const SAttribute* getAttributeByName(
						const char_type* name) const
				{
					if (!name)
						return 0;
						...
						core::string<char_type>& origstr)
				{
					...
					while (pos != -1 && pos < (int) origstr.size() - 2)
					{
						...
						for (int i = 0; i < (int) SpecialCharacters.size(); ++i)
						{
							...
							if (equalsn(&SpecialCharacters[i][1], p,
						...
						{
							newstr.append(
									origstr.subString(oldPos, pos - oldPos));
				...
				void convertTextData(src_char_type* source,
						char* pointerToStore, int sizeWithoutHeader)
				{
					// convert little to big endian if necessary
					if (sizeof(src_char_type) > 1
							&& isLittleEndian(TargetFormat)
									!= isLittleEndian(SourceFormat))
				...
				bool equalsn(const char_type* str1, const char_type* str2,
						int len)
				{
					int i;
