...
namespace irrgame
{
	namespace video
	{
		...
		static void executeBlit_TextureBlend_16_to_16(const SBlitJob * job)
		{
			...
			const u32 off = core::if_c_a_else_b(job->width & 1, job->width - 1,
					0);
			...
			for (dy = 0; dy != job->height; ++dy)
			{
				for (dx = 0; dx != rdx; ++dx)
				{
					dst[dx] = video::PixelBlend16_simd(dst[dx], src[dx]);
				}

				if (off)
				{
					((u16*) dst)[off] = video::PixelBlend16(((u16*) dst)[off],
							((u16*) src)[off]);
				}
		...
		static void executeBlit_TextureBlend_32_to_32(const SBlitJob * job)
		{
			...
			for (s32 dy = 0; dy != job->height; ++dy)
			{
				for (s32 dx = 0; dx != job->width; ++dx)
				{
					dst[dx] = video::PixelBlend32(dst[dx], src[dx]);
				}
				src = (u32*) ((u8*) (src) + job->srcPitch);
				dst = (u32*) ((u8*) (dst) + job->dstPitch);
			}
		...
		static void executeBlit_TextureBlendColor_16_to_16(const SBlitJob * job)
		{
			...
			for (s32 dy = 0; dy != job->height; ++dy)
			{
				for (s32 dx = 0; dx != job->width; ++dx)
				{
					...
					dst[dx] = video::PixelMul16_2(src[dx], blend);
				}
				src = (u16*) ((u8*) (src) + job->srcPitch);
				dst = (u16*) ((u8*) (dst) + job->dstPitch);
			}
		...
		static void executeBlit_TextureBlendColor_32_to_32(const SBlitJob * job)
		{
			...
			for (s32 dy = 0; dy != job->height; ++dy)
			{
				for (s32 dx = 0; dx != job->width; ++dx)
				{
					dst[dx] = video::PixelBlend32(dst[dx],
							video::PixelMul32_2(src[dx], job->argb));
				}
				src = (u32*) ((u8*) (src) + job->srcPitch);
				dst = (u32*) ((u8*) (dst) + job->dstPitch);
			}
		...
		static void executeBlit_Color_16_to_16(const SBlitJob * job)
		{
			...
			if (0 == (job->srcPitch & 3))
			{
				for (s32 dy = 0; dy != job->height; ++dy)
				{
					video::memset32(dst, c, job->srcPitch);
					dst = (u16*) ((u8*) (dst) + job->dstPitch);
				}
			}
			else
		...
		static void executeBlit_Color_32_to_32(const SBlitJob * job)
		{
			...
			for (s32 dy = 0; dy != job->height; ++dy)
			{
				video::memset32(dst, job->argb, job->srcPitch);
				dst = (u32*) ((u8*) (dst) + job->dstPitch);
			}
		}
		...
		static void executeBlit_ColorAlpha_16_to_16(const SBlitJob * job)
		{
			...
			for (s32 dy = 0; dy != job->height; ++dy)
			{
				for (s32 dx = 0; dx != job->width; ++dx)
				{
					dst[dx] = 0x8000 | video::PixelBlend16(dst[dx], src, alpha);
				}
				dst = (u16*) ((u8*) (dst) + job->dstPitch);
			}
		}
			...
			for (s32 dy = 0; dy != job->height; ++dy)
			{
				for (s32 dx = 0; dx != job->width; ++dx)
				{
					dst[dx] = (job->argb & 0xFF000000)
							| video::PixelBlend32(dst[dx], src, alpha);
				}
				dst = (u32*) ((u8*) (dst) + job->dstPitch);
			}
		}
		...
		static inline tExecuteBlit getBlitter2(eBlitter operation,
